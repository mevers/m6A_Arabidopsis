...
par(mfrow = c(2,1));
PlotSpatialDistribution(posSites, filter = c("CDS", "3'UTR"),
                        absolute = TRUE, binWidth = 20);
PlotSpatialDistribution(negSites, filter = c("CDS", "3'UTR"),
                        absolute = TRUE, binWidth = 20);
dev.off();
...
motifSites <- GetMotifLoc(motif, filter = c("CDS", "3'UTR"),
                          refGenome = "TAIR10");
...
PlotRelDistEnrichment(posSites, negSites, motifSites,
                      flank = 205, binWidth = 10);
dev.off();
...
PlotRelDistEnrichment(posSites, negSites, PACsites, flank = 205, binWidth = 10);
dev.off();
...
PlotRelDistDistribution(posSites, PACsites, flank = 205, binWidth = 10);
dev.off();
pdf("figures/relDistAbundance_null-m6A_PACsites.pdf", width = 15, height = 10);
PlotRelDistDistribution(negSites, PACsites, flank = 205, binWidth = 10, doBootstrap = FALSE);
dev.off();

############################################
#### m6A - cleavage site colocalisation ####
############################################
...
PlotRelDistEnrichment(posSites, negSites, Csites, flank = 205, binWidth = 10);
dev.off();
...
PAC_Thomas_KD <- SmartMap(sites, id = "PAC_CPSF30-KD", refGenome = "TAIR10");
PAC_Thomas_KD <- FilterTxLoc(PAC_Thomas_KD, filter = c("CDS", "3'UTR"));
...
# Plot distribution of APA sites across transcript sections
pdf("figures/sectionAbundance_PAC_Thomas2012.pdf", height = 5, width = 7.5);
PlotSectionDistribution(PAC_Thomas_WT);
PlotSectionDistribution(PAC_Thomas_KD);
PlotSectionDistribution(PAC_Thomas_common);
dev.off();

# Plot enrichment of relative distances m6A/non-m6A to PAC sites in tx
pdf("figures/relDistEnrichment_m6A_relative_to_PACsites_Thomas2012.pdf", width = 15, height = 10);
PlotRelDistEnrichment(posSites, negSites, PAC_Thomas_WT, flank = 205, binWidth = 10);
PlotRelDistEnrichment(posSites, negSites, PAC_Thomas_KD, flank = 205, binWidth = 10);
PlotRelDistEnrichment(posSites, negSites, PAC_Thomas_common, flank = 205, binWidth = 10);
dev.off();
...
PlotOverlap(FilterTxLoc(posSites, filter = c("CDS", "3'UTR")), PAC_Thomas_KD);
dev.off();

# Plot enrichment of relative distances PAC sites to m6A sites in tx
pdf("figures/relDistEnrichment_PACsites_relative_to_m6A_Thomas2012.pdf", width = 15, height = 10);
PlotRelDistEnrichment(PAC_Thomas_WT, PAC_Thomas_common, posSites, flank = 205, binWidth = 10);
PlotRelDistEnrichment(PAC_Thomas_KD, PAC_Thomas_common, posSites, flank = 205, binWidth = 10);
dev.off();
